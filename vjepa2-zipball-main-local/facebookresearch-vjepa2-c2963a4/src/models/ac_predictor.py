...
class VisionTransformerPredictorAC(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # --- write a hierarchical summary of the predictor modules to output/
        try:
            repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
            out_dir = os.path.join(repo_root, "output")
            os.makedirs(out_dir, exist_ok=True)
            timestamp = int(time.time())
            fname = os.path.join(out_dir, f"ac_predictor_structure_{timestamp}.md")

            def write_module_tree(fh, module, name="self", indent=0):
                indent_s = "  " * indent
                try:
                    params = sum(p.numel() for p in module.parameters(recurse=False))
                except Exception:
                    params = 0
                fh.write(f"{indent_s}- {name}: {module.__class__.__name__} (params: {params})\n")
                for child_name, child in module.named_children():
                    write_module_tree(fh, child, child_name, indent + 1)

            with open(fname, "w") as fh:
                fh.write("# VisionTransformerPredictorAC module structure\n\n")
                total_params = sum(p.numel() for p in self.parameters())
                fh.write(f"- class: {self.__class__.__name__}\n")
                fh.write(f"- total_params: {total_params}\n\n")
                fh.write("## Modules (top-down)\n\n")
                write_module_tree(fh, self, name="self", indent=0)
                fh.write("\n## Repr\n\n```")
                fh.write(repr(self))
                fh.write("\n```\n")

            print(f"Wrote AC predictor structure to {fname}")
        except Exception as _e:
            print(f"Could not write AC predictor structure: {_e}")
