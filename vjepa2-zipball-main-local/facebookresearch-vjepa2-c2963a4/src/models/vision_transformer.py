...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # --- write a structured summary of the instantiated model to output/
        try:
            repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
            out_dir = os.path.join(repo_root, "output")
            os.makedirs(out_dir, exist_ok=True)
            timestamp = int(time.time())
            fname = os.path.join(out_dir, f"vision_transformer_structure_{timestamp}.md")
            ...
            structure = {
                ...
                "num_patches": getattr(self, "num_patches", None),
                "num_layers": self.get_num_layers(),
                "total_params": sum(p.numel() for p in self.parameters()),
            }

            # per-module (top-level) parameter counts (non-recursive)
            modules = {}
            for name, module in self.named_children():
                params = sum(p.numel() for p in module.parameters(recurse=False))
                modules[name] = params
            structure["modules_param_counts"] = modules

            # Also include a short textual repr
            structure["repr"] = repr(self)

            # Write as a human-readable Markdown file instead of JSON
            with open(fname, "w") as fh:
                fh.write("# VisionTransformer structure\n\n")
                # Basic key/value pairs
                fh.write("## Metadata\n\n")
                fh.write(f"- class: {structure.get('class')}\n")
                fh.write(f"- embed_dim: {structure.get('embed_dim')}\n")
                fh.write(f"- patch_size: {structure.get('patch_size')}\n")
                fh.write(f"- num_frames: {structure.get('num_frames')}\n")
                fh.write(f"- tubelet_size: {structure.get('tubelet_size')}\n")
                fh.write(f"- num_patches: {structure.get('num_patches')}\n")
                fh.write(f"- num_layers: {structure.get('num_layers')}\n")
                fh.write(f"- total_params: {structure.get('total_params')}\n\n")

                fh.write("## Module parameter counts (top-level, non-recursive)\n\n")
                for mname, cnt in structure.get('modules_param_counts', {}).items():
                    fh.write(f"- {mname}: {cnt}\n")
                fh.write("\n")

                fh.write("## Module repr\n\n")
                fh.write("```")
                fh.write(structure.get('repr', ''))
                fh.write("\n```)\n")

            # Print a short confirmation to stdout
            print(f"Wrote VisionTransformer structure to {fname}")
        except Exception as _e:
            # Do not raise errors during model construction for robustness
            print(f"Could not write VisionTransformer structure: {_e}")
